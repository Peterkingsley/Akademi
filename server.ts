...
async function startServer() {
  ...
  app.post('/api/user', (req, res) => {
    const { name, email, university, department, level } = req.body;
    // For this demo, we'll just update the hardcoded user_1 or create if not exists
    const existingUser = db.prepare('SELECT * FROM users WHERE id = ?').get('user_1');
    
    if (existingUser) {
      db.prepare('UPDATE users SET name = ?, email = ?, university = ?, department = ?, level = ? WHERE id = ?')
        .run(name, email, university, department, level, 'user_1');
    } else {
      db.prepare('INSERT INTO users (id, name, email, university, department, level) VALUES (?, ?, ?, ?, ?, ?)')
        .run('user_1', name, email, university, department, level);
    }
